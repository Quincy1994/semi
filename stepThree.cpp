...
void LoadLabelVector()
{
    long long num_vertices, vector_dim, a, b;
    char l_name[MAX_STRING], ch;
    int vid_l;
    real *vec;
    FILE *fi;
    if(l_emb_file == NULL){
        printf("Error: label embedding file not found\n");
        exit(1);
    }
    fi = fopen(l_emb_file, "rb");
    real value;
    fscanf(fi, "%lld %lld", &num_vertices, &vector_dim);
    vec = (real *)malloc(vector_dim * sizeof(real));
    for (a = 0; a < num_vertices; a++){
        fscanf(fi, "%s%c", l_name, &ch);
        for(b = 0; b < vector_dim; b++){
            fscanf(fi, "%f ", &value);
            vec[b] = value;
        }
...
int main(int argc, char ** argv) {
    ...
    if (argc == 1){
        ...
        printf("\t-labelemb <file>\n");
        printf("\t\tUse <file> to save the learnt label embeddings\n");
    ...
    if ((i = ArgPos((char *)"-ulUpdateNum", argc, argv)) > 0) ul_update_num = atoi(argv[i + 1]);
